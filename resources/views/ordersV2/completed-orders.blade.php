...
@section('content')
    <x-adminlte.page-header :title="__('Completed Orders V2')" subtitle="Browse finalized updated-order records and printable invoice links.">
        <x-slot:actions>
            <div class="btn-group">
                <a href="{{ route('ordersV2.index') }}" class="btn btn-default">{{ __('All Orders') }}</a>
    ...
    <x-adminlte.page-body container-class="container container-xl">
        ...
        @if ($orders->isEmpty())
            <x-empty title="No orders found"
                message="Completed orders will appear here after pending updated-order entries are finalized."
                button_label="{{ __('Add your first Order') }}" button_route="{{ route('ordersV2.create') }}" />
        @else
            <x-card>
                            ...
                            <tbody>
                                @foreach ($orders as $order)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <a href="{{ route('ordersV2.show', $order) }}"
